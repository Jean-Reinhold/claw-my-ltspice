...
def create_circuit(include_path: str = "../../lib/claw_opamps.lib") -> Circuit:
    ...
    circuit.voltage("VIN1", "in1", "0", "PULSE(0 0.5 0 1u 1u 5m 10m)", at=(96, 176))
    circuit.voltage("VIN2", "in2", "0", "PULSE(0 0.25 1m 1u 1u 5m 10m)", at=(96, 336))
    circuit.voltage("VCC", "vcc", "0", "5", at=(288, 48))
    circuit.voltage("VEE", "vee", "0", "-5", at=(288, 400))
    circuit.resistor("R1", "in1", "sum", "10k", at=(224, 256))
    circuit.resistor("R2", "in2", "sum", "20k", at=(224, 352))
    circuit.resistor("RF", "out", "sum", "20k", at=(464, 128))
    circuit.opamp("XU1", "0", "sum", "vcc", "vee", "out", at=(416, 208))
    circuit.resistor("RL", "out", "0", "10k", at=(640, 272), rotation="R90")
    circuit.wire(64, 176, 160, 176)
    circuit.wire(160, 176, 160, 256)
    circuit.wire(160, 256, 224, 256)
    circuit.wire(64, 336, 160, 336)
    circuit.wire(160, 336, 160, 352)
    circuit.wire(160, 352, 224, 352)
    circuit.wire(320, 256, 352, 256)
    circuit.wire(320, 352, 352, 352)
    circuit.wire(352, 256, 352, 304)
    circuit.wire(352, 352, 352, 304)
    ...
    circuit.iopin(64, 176, "in1", "In")
    circuit.iopin(64, 336, "in2", "In")
    circuit.iopin(704, 272, "out", "Out")
    circuit.flag(96, 272, "0")
