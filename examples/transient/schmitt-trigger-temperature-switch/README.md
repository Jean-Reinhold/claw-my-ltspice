...
The documentation plot marks the `fan on` and `fan off` thresholds with dashed
reference lines. The filtered `V(sense)` trace should cross those lines once per
slow temperature ramp, while `V(sensor_raw)` visibly carries more ripple.
