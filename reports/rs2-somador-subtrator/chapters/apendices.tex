...
(ensaio global com a referência comportamental ideal):
...
VCC vcc 0 15
VEE vee 0 -15
BIDEAL ideal 0 V=112.5*V(in1)+35*V(in2)-31.25*V(in3)+25*V(in4)+50*V(in5)-112.5*V(in6)
.tran 0 20m 0 10u
...
VCC vcc 0 15
VEE vee 0 -15
BIDEAL1 ideal1 0 V=-9*V(in1)-2.8*V(in2)+2.5*V(in3)
.tran 0 20m 0 10u
...
VIN6 in6 0 0
.tran 0 5m 0 5u
.meas TRAN g_out_vin1 FIND V(out)/50m AT=2.25m
.meas TRAN g_vout1_vin1 FIND V(vout1)/50m AT=2.25m
.meas TRAN g_vout2_vin1 FIND V(vout2)/50m AT=2.25m
.meas TRAN vout_max MAX V(out)
\end{verbatim}
}
...
{\small
\begin{verbatim}
.include lm741_vendor.lib          (macromodelo de Boyle da TI)
...
O integrador Gear e as opções de convergência foram necessários porque
o macromodelo de Boyle oscila numericamente com o integrador
trapezoidal padrão; 10 ms cobrem um período completo do estímulo
multi-tom (fundamental de \SI{100}{\hertz}).
